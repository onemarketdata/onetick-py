...
def write_parquet(
    ...
    **kwargs,
    ...
    if 'num_tick_per_row_group' in kwargs:
        warnings.warn(
            "Parameter 'num_tick_per_row_group' is deprecated by OneTick and will be ignored",
            FutureWarning
        )
...
def write_iceberg(
    ...
    *,
    ...
    propagate_input_ticks: bool = False,
    fields_for_disabled_dictionary_encoding: list[str] | None = None,
    inplace: bool = False,
):
    """
    ...
    an internal :meth:`~onetick.py.Source.write_parquet` method.
    ...
    table_identifier: str
        The Iceberg table identifier, including the namespace and table name (for example, ``namespace.tablename``).
        The table must already exist.
    output_path: str
        The output location for the parquet data files.
        This value is passed through to the internal :meth:`~onetick.py.Source.write_parquet` method;
    ...
    memory_threshold: float
        How much, in megabytes, is buffered per thread before ticks are passed to be written.
        Default: 50
    propagate_input_ticks: bool
        If True then ticks will be propagated after this method.
    ...
    >>> data = otp.Tick(A=1)                                                # doctest: +SKIP
    >>> data = data.write_iceberg(catalog_config='catalog.cfg',             # doctest: +SKIP
    ...                           table_identifier='reporting.sales_data',  # doctest: +SKIP
    ...                           output_path='sales')                      # doctest: +SKIP
    >>> otp.run(data)                                                       # doctest: +SKIP
    """
    compression_type = compression_type.upper()
    if not fields_for_disabled_dictionary_encoding:
        fields_for_disabled_dictionary_encoding_str = ''
    elif isinstance(fields_for_disabled_dictionary_encoding, str):
        fields_for_disabled_dictionary_encoding_str = fields_for_disabled_dictionary_encoding
    else:
        fields_for_disabled_dictionary_encoding_str = ','.join(fields_for_disabled_dictionary_encoding)
    ...
    self.sink(
        otq.WriteToIceberg(
            ...
            propagate_input_ticks=propagate_input_ticks,
            fields_for_disabled_dictionary_encoding=fields_for_disabled_dictionary_encoding_str,
        )
    )
